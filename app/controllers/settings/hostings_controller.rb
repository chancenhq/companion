class Settings::HostingsController < ApplicationController
  ...
  def update
    ...
    if hosting_params.key?(:metabase_url)
      Setting.metabase_url = hosting_params[:metabase_url]
    end

    update_encrypted_setting(:metabase_api_key)

    if hosting_params.key?(:metabase_student_question_id)
      Setting.metabase_student_question_id = hosting_params[:metabase_student_question_id]
    end

    if hosting_params.key?(:metabase_email_param)
      Setting.metabase_email_param = hosting_params[:metabase_email_param].presence || "email"
    end
